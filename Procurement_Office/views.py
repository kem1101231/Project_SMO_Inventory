...
def quotation(request):
	...
	for x in quoList:
		compNums = len(reqQ.getReqComp(x[0]))
		prProcDetails = preq.getProCTimeLineDetails(x[1])
		refString = x[0]
		toPageData = toPageData + [(x, prProcDetails[0],(compNums, refString.replace('-','_'))),]
...
def suppliers(request):
	...
	supCompList = sup.getAllSuppliers()

	return setReponse(request, 'procurement/suppliers.html', supCompList,[],[],[],[])		
...
def allPR(request):
	...
	for x in allPRnum:
		...
		if prDetails[0][1] != None:
		 	prProcList = preq.getPRNumforPONumForNew(prDetails[0][1]) 
		...
		if len(prProcList) > 1:
			...
			for y in prProcList:
				...
				prProcTimeDetails = preq.getProCTimeLineDetails(y[0])
			
				toPageData = toPageData + [(prDetails[0]+empDetails[0]+(offDetails[1], str(y[0]).replace('-','_')) + prProcTimeDetails[0]),]


		else:

			prProcTimeDetails = preq.getProCTimeLineDetails(x[0])
			toPageData = toPageData + [(prDetails[0]+empDetails[0]+(offDetails[1], preqNum.replace('-','_')) + prProcTimeDetails[0]),]
...
def allPR(request):
	...
	toPageData = f.getAllPRListByStatus('approved', 'final')
...
def numPR(request):
	...
	if request.method == 'POST':
		prNumInput = request.POST['prNumInput']
		reqnum = request.POST['reqnum']
		#prdate = request.POST['prDate']

		f.numberPRFromInput(reqnum, prNumInput)
		preq.updatePrNum(reqnum, prNumInput, '')
		preq.updatePRLoc(reqnum, 'in', currentdate, 'proc')

	return HttpResponse('')		

def addQuotNum(request):
	...
	if request.method == 'POST':
		rqNumInput = request.POST['rqNumInput']
		reqnum = request.POST['reqnum']
		rqdate = request.POST['rqDate']

		print( rqNumInput +" /// "+ str(rqdate) + " /// "+ reqnum)	
		
		preq.updateQuotNum(reqnum, rqNumInput, rqdate)
		f.addReqQuoWithRQNumIn(rqNumInput, reqnum, '', '', '', [], [])

	return HttpResponse('')	

def updateQuotationInfo(request):
	...
	if request.method == 'POST':
		reqnum = request.POST['reqnum']
		rqDateStart = request.POST['rqDateStart']
		rqDateColl = request.POST['rqDateColl']
		bidNum = request.POST['bidNum']
		bidList = json.loads(request.POST.get('bidList'))

		quotNum = reqQ.getReqNumFromRefNum(reqnum)
		preq.updateCanvassStartEnd(reqnum, rqDateStart, rqDateColl, bidNum)
		f.updateSuppliersOfRQ(quotNum,bidList)

	return HttpResponse('')	

def addABCInfo(request):
	...
	if request.method == 'POST':
		reqnum = request.POST['reqnum']
		abcnum = request.POST['abcnum']
		dFlag = request.POST['dFlag']
		abcreldate = request.POST['abcreldate']
		abcretdate = request.POST['abcretdate']

		print('-------------------------------')
		print(abcnum)

		if abcnum != '':
			preq.addACData(reqnum, abcnum, abcreldate)

			if dFlag == 'true':
				f.addAbsCanv(abcnum, abcreldate, '', [])
		
		else:
			preq.updateACData(reqnum, abcretdate)

			

	return HttpResponse('')	

def addPOData(request):
	...
	if request.method == 'POST':
		...
		porelsigndate = request.POST['porelsigndate']

		if "$" in ponum:
			poList = ponum.split('$')
			f. addMultiPOList(poList, reqnum)
		
		else:
			preq.addMultiPO(reqnum, ponum)
			f.createPO( ponum, '', '', porelsigndate, '', '0', '', 0, '', reqnum, [])
		
		preq.addPOData(reqnum, ponum, porelsigndate)
	
	return HttpResponse('')	

def updatePOApprovalData(request):
	...
	if request.method == 'POST':
		reqnum = request.POST['reqnum']
		appstatus = request.POST['appstatus']
		poretsigndate = request.POST['poretsigndate']
		
		preq.updatePOAppData(reqnum, appstatus, poretsigndate)

	return HttpResponse('')	

def updatePOServeData(request):
	...
	if request.method == 'POST':
		reqnum = request.POST['reqnum']
		poserdate = request.POST['poserdate']
		expdeldate = request.POST['expdeldate']
		supplierName = request.POST['supplier']

		poserdateArr = poserdate.split('$')
		supplierNameArr = supplierName.split('$')
		expdeldateArr = expdeldate.split('$')

		if "$" in supplierName:
			for idx, x in enumerate(supplierNameArr):
				preq.updatePOSerData(reqnum, poserdateArr[idx], expdeldateArr[idx], x)
				preq.updateMulti({'supplier':(x,'String'), 'expdeldate':(expdeldateArr[idx],'Date'), 'posrstatusdate':(poserdateArr[idx],'Date')}, reqnum)

		else:
			preq.updatePOSerData(reqnum, poserdate, expdeldate, supplierName)
			preq.updateMulti({'supplier':(supplierName,'String'), 'expdeldate':(expdeldate,'Date'), 'posrstatusdate':(poserdate,'Date')}, reqnum)
...
def prDetails(request, refData):
	...
	reqNumString = refData.replace('_','-')
	reqNumArr = refData.split('_')

	reqNum = reqNumArr[0]+"-"+reqNumArr[1]

	prDetails = preq.getPRDetails(reqNum)
	prProcLocDetails = preq.getProCTimeLineDetails(reqNumString)
	
	print(prProcLocDetails[0][12] != None and "$" in prProcLocDetails[0][12])

	if prProcLocDetails[0][12] != None and "$" in prProcLocDetails[0][12]:
		multiPOList = preq.getMultiPONum(reqNum)
		print(multiPOList)
		
		prProcLocDetails[0] = prProcLocDetails[0] + (multiPOList,)		


	prLocDetails = preq.getPRLocDetails(reqNum)

	empDetails = e.getEmployeeDetails(prDetails[0][4])
	offDetails = off.getOfficeDetails(empDetails[0][4])

	if len(reqNumArr) == 3:
		ceString = '(CE-'+str(reqNumArr[2])+')'

	prTotalCost = preq.getPRTotalCost(reqNum)	

	if prTotalCost[0][0] > 500000:
		return setReponse(request, 'procurement/pr500_details.html', [prDetails[0], prProcLocDetails[0], empDetails[0], (offDetails[1],), prLocDetails[0]], [(reqNumString, ceString),], [], [],[])	
	
	else:	
		return setReponse(request, 'procurement/pr_details_new.html', [prDetails[0], prProcLocDetails[0], empDetails[0], (offDetails[1],), prLocDetails[0]], [(reqNumString, ceString),], [], [],[])


''' # the Original Function
def prDetails(request, refData):

	reqNum = refData.replace('_','-')

	prDetails = preq.getPRDetails(reqNum)
	prProcLocDetails = preq.getProCTimeLineDetails(reqNum)
	prLocDetails = preq.getPRLocDetails(reqNum)
	...
	return setReponse(request, 'procurement/pr_details.html', [prDetails[0], prProcLocDetails[0], empDetails[0], (offDetails[1],), prLocDetails[0]], [], [], [],[])
'''	
...
def rqCompleteDetailsFromRef(request, refData):
	...
	detailsToPage = f.getRQCompleteDetailsWithItemss(rqNum)
	rqItems = reqQ.getReqItems(rqNum)

	for idx, x in enumerate(rqItems):
			
		if "$" in x[2]:
			xList = list(x)
			xList[2] = ('multi',str(x[2]).split('$'))

			xTuple = tuple(xList)

			rqItems[idx] = xTuple

	return setReponse(request, 'procurement/rq_details.html', detailsToPage, rqItems,[],[],[])	
...
def addReqQuotation(request):
	...
	if ceNum != '':
		prNumtoAdd = str(reqNum[0][0])+'-'+ceNum
	else:
		prNumtoAdd = str(reqNum[0][0])
			

	f.addReqQuoWithRQNumIn(quotNumCode, prNumtoAdd, projName, projLoc, canv, [], itemsMatrix)
	preq.updateQuotNum(prNumtoAdd, quotNumCode, datetime.now().strftime('%Y-%m-%d'))
	...
	return HttpResponse(prNumtoAdd)
...
def addAbstractOfCanvass(request):
	...
	print(abcnum)
	quoDetails = reqQ.getRequestDetails(quotNum)
	preq.addACData(quoDetails[0][1], abcnum, '')
	f.addAbsCanv(quotNum, openingDate, openingTime, theArray)

	return HttpResponse((quoDetails[0][1]).replace('-','_'))	
...
def createNewTimeline(request):
	
	if request.method == 'POST':
		
		reqnum = request.POST['reqnum']
		prnum = request.POST['prnum']
		prnumdate = request.POST['prnumdate']

		reqnumNew = f.createNewTimeline(reqnum, prnum, prnumdate)
		
	return HttpResponse(reqnumNew)	
...
def generateRQDocumentWithRef(request, refData):
		
		quoNumber = refData.replace('_','-')
		...
		toLoopRef = 19 - minLoop
		loopRef = genLoopRangeString(toLoopRef, rqItems)
		

		return renderPDF(request,'pdf_templates/doc-rqTest.html', [('','', procOffHead.upper()),(quoDetails[0][0], str(quoDetails[0][4]), cansvasserName.upper(), quoDetails[0][2], quoDetails[0][3],quoDetails[0][1],),], rqItems,[],[], str(quoNumber)+'_'+str(1), loopRef, 'legal')

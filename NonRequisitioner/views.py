...
def index(request):
	...
	return setReponse(request, 'non_requisitioner/index_new.html', [('rr','tt','yy','uu'),('ii','qq','ww'),('ss','dd')],[(loopRef,),],[],[],[],{})
...
def search(request):
	...
	return setReponse(request, 'non_requisitioner/search.html', result,[],[],[],[],{})
...
def profile(request):	
	...
	return setReponse(request, 'non_requisitioner/profile.html',userdata,[(allPRNum,offDetails[1]),parStat],empdata,[],[],{})
...
def updateProfilePic(request):	
	...
	if request.method == 'POST':
		...
		#prType = request.POST['thePicData']
...
def inquire(request):
	...
	return setReponse(request, 'non_requisitioner/inq_items.html',[],[],[],[],[],{})
...
def ris(request):
	...
	return setReponse(request, 'non_requisitioner/ris.html', toPageData,[],[],[],[],{})	
...
def risDetails(request):
	...
	return setReponse(request, 'non_requisitioner/ris_details.html',risDetails2, [(reqOfficerName, reqOff[1])], risItems, [],[],{})
...
def create_pr(request):
	...
	if isSessionValueEmpty(request,'createPRCookie'):
		response = setReponse(request, 'non_requisitioner/create_pr_new.html',[],[],[],[('#','$String1|!String2;!String3;!String4;','Unit','Qty', 'Price', 'Total'),('#','$String1|!String2;!String3;!String4;','Unit','Qty','Price', 'Total'),('#','$String1|!String2;!String3;!String4;','Unit','Qty', 'Price', 'Total'),('#','String1','Unit','Qty', 'Price', 'Total')],[],{})
		...
		response = setReponse(request, 'non_requisitioner/create_pr_new.html', prDetails, prItems,[(len(prItems), prTotalCost),],[('$String1;String2;String3;String4;',),],[],{})
...
def pr(request):
	...
	prList = f.getPRListByStaffPersonnel(logDetails[0][1])
	prStat = f.getReqStat(logDetails[0][1])

	return setReponse(request, 'non_requisitioner/pr.html', prList, prStat,[],[],[],{})

def theOffDetails(request):
	
	return setReponse(request, 'non_requisitioner/off_details.html', [], [], [],[],[],{})
...
def accounts(request):
	...
	return setReponse(request, 'non_requisitioner/accounts.html', outputItemList, [parStat,],[],[],[],{})
...
def abstract(request):
	...
	return setReponse(request, 'non_requisitioner/abstract.html', absDetails, suppliersComplete, toPageItems, prDetails,[],{})
...
def prDetails(request):
	...
	response = setReponse(request, 'non_requisitioner/pr_details.html', prDetails, prItemstoPage,[(totalCostOfPR,'', loopRef),],[chanData[0],employeeData[0]],[(prLocDetails[0],)],prStat)
...
def prDetails_WithRef(request, refData):
	...
	response = setReponse(request, 'non_requisitioner/pr_details.html', [prDetails[0]], prItemstoPage,[(totalCostOfPR,'', loopRef),],[chanData[0],employeeData[0]],[prLocDetails[0],prPOTimeline[0], iarInfoToPage, acctInfoToPage],prStat)
...
def parDetails(request):
	...
	response = setReponse(request, 'non_requisitioner/par_details.html', parDetails, parItems,[(parTotal,receiveFrom, stringInt),],poDetails,[],{})
	return response
...
def addPr(request):
	...
	f.createPRStaff(reqnum, purpose, prType, logDetails[0][1], itemList)
...
def notifications(request):
	 ...
	 if notifRefType == 'ris':
	 	...
	 	response = HttpResponseRedirect('/non_requisitioner/ris/details/')	
...
def setReponse(request, outputPage, pageData1, pageData2, pageData3, pageData4, pageData5, dictData1):
	...
	if isSessionValueEmpty(request,'theCookie'):
		...
		if any( 4 == code for code in acctType):

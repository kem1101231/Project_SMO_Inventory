...
urlHead = '/inventory_office_admin'

def index(request):
    return setReponse(request, templateFolder+'/home.html', [], [], [], [], [])
...
def abstract(request):
    ...
    if isSessionValueEmpty(request,'prCookie'):
        ...
        for x in suppliers:
            supDetails = suppl.getSupllierDetails(x[0])
    ...
    return setReponse(request, 'inventory_office_admin/abstract.html', absDetails, suppliersComplete, toPageItems, prDetails,[])
...
def updateAbstract(request):
    ...
    return HttpResponseRedirect('/inventory_office_admin/pr_smo/')
...
def risSMO(request):
    ...
    risList = ris_Class.getAllRISbyID(logDetails[0][1])

    for x in risList:
        risDetails = ris_Class.getRISDetails(x[0])

        toPageData = toPageData + risDetails



    return setReponse(request, 'inventory_office_admin/ris_smo.html', toPageData,[],[],[],[]) 

def risSMODetails(request):

    slipnum = getSessionData(request, 'theSlipNum')

    risDetails2 = ris_Class.getRISDetails(slipnum)
    ...
    return setReponse(request, 'inventory_office_admin/ris_smo_details.html',risDetails2, [(reqOfficerName, reqOff[1])], risItems, [],[])

def displayRISSMODetails(request):
    
    slipnum = request.POST.get('slipNumButton', '')
    setSessionData(request, 'theSlipNum', slipnum)
    
    return HttpResponseRedirect('/inventory_office_admin/ris_smo/details/') 
...
def create_pr(request):
    ...
    if isSessionValueEmpty(request,'createPRCookie'):
        response = setReponse(request, 'inventory_office_admin/create_pr_smo_new.html',[],[],[],[],[])
        ...
        response = setReponse(request, 'inventory_office_admin/create_pr_smo_new.html', prDetails, prItems,[(len(prItems), prTotalCost),],[],[])
...
def prSMO(request):
    ...
    return setReponse(request, 'inventory_office_admin/pr_smo.html', prList, prStat,[],[],[])

def prSMODetails(request):
    ...
    response = setReponse(request, 'inventory_office_admin/pr_smo_details.html', prDetails, prItems,[(totalCostOfPR,'', loopRef),],[chanData[0],employeeData[0]],prStat)
    
    return response
def inquire(request):
    
    return setReponse(request, 'inventory_office_admin/inq_items.html',[],[],[],[],[])
...
def displayReqDetails(request):
    ...
    response = HttpResponseRedirect('/inventory_office_admin/pr_smo/pr_details')
...
def createPRSMOFromRef(request):
     
    reqnum = request.POST.get('detailsButton','')
    response = HttpResponseRedirect('/inventory_office_admin/create_pr')
...
def getSupplyAvailability(request):
    ...
    if request.method == 'POST':
        ...
        outputData = {
                    'unit':itemDetails[0][4].lower(),
                    'quantity': str(totalAvailable),
                    }
...
def receive(request):
    ...
    for x in iarNumList:
        ...
        poDetails_2 = pOrd.getPODetails(iarDetails[0][1])
        ...
        dataToPage = dataToPage + [(poDetails_2[0][18], iarNum, iarDetails[0][10], delStat, compStat, iarNum.replace('-','_')),]
...
def generatePRDocument(request):
        ...
        offName = off.getOfficeDetails(employeeData[0][4])
        loopRef = genLoopRangeString(26, prItems)
...
def generateRISDocument(request):
        ...
        smoHead = off.getOfficeHeadFromOffice('SMO')
...
def notifications(request):
     ...
     if notifRefType == 'pr':
        response = HttpResponseRedirect(urlHead+'/pr_smo/pr_details')
        n.updateNotif(notifNum)

     if notifRefType == 'abstract':
        response = HttpResponseRedirect(urlHead+'/abstract_details')  
...
def setReponse(request, outputPage, pageData1, pageData2, pageData3, pageData4, pageData5):
    ...
    if isSessionValueEmpty(request,'theCookie'):
        ...
        c = {
            ...
            'pageData4': pageData4,
            'pageData5': pageData5,
            'baseSource': 'inventory_office_admin/base.html',
            'urlHead': urlHead,
            'cssActiveMenu' : 'active-menu-inven',
        
        }

        if any( 3 == code for code in acctType):
